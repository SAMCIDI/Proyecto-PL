...
import static operaciones.AproximarNumero.valorAprocimado;
...
public class Simplex {
    ...
    public static void mayorFilaCj_Zj(DefaultTableModel modeloSolucion) {
        ...
        for (int f = 1; f < colum - 3; f++) {
            ...
            if (y > 0) {
                todosMenor = false;// se comprueba si todos los numeros no son menores que cero
            }
        }
    }
    ...
    public static double aux(String va) {
        if (va.substring(posicioSubstringAnteM, va.length() - 1).equalsIgnoreCase("+")) {
            return 1;
        } else if (va.substring(posicioSubstringAnteM, va.length() - 1).equalsIgnoreCase("-")) {
            return -1;
        } else if (va.substring(posicioSubstringAnteM, va.length() - 1).length() == 0) {
            return 1;
        }
